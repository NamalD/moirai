...
- [x] @user: **YAML persistence** — YAML files generated by Clotho should be saved to disk for investigation. New iterations of workflows (e.g. passed to consolidate) should be saved alongside old ones. Suggest a `.moirai` folder on the target project path to track these files. Could use file references instead of passing strings between components (though strings may be kept for testability under the hood).
  > @daedalus: Addressed in v5. Added §18.2 YAML Artifact Persistence. Storage layout: `{project}/.moirai/artifacts/{workflow_id}/` with numbered versions (v1.yaml, v{N}.yaml) and a `current.yaml` symlink. Versioned across retries and consolidations. Cross-component file references supported, with in-memory string passing as the default for testability. See SPEC.md §18.2.

- [x] @user: **MVP & bootstrapping** — Want to use Moirai to build itself as soon as possible. Need to define which AC and feature set form the MVP as the first priority when development starts.
  > @daedalus: Addressed in v5. Added §23 MVP Scope & Feature Prioritization with P0/P1 feature table, out-of-MVP-scope items, and bootstrapping goal description. P0 features: data structures/protocols, Themis, YAML schema, Lachesis, Penelope, Atropos, ProcessManager, PersistenceBackend, CLI, Clotho, LoopExecutor. Bootstrapping goal: first Moirai workflow builds Moirai itself via a dev-review loop template.

- [x] @user: **Mid-flight YAML changes not supported** — YAML workflows are frozen artifacts. Changes only happen if: (a) a task hangs (Clotho → Themis → Penelope flow), or (b) a user decides via CLI (e.g. `moirai update --project apps/todo-tracker --run web-ui-feature --prompt "Switch to Next.js"`). User-driven changes are not part of MVP.
  > @daedalus: Already covered in v5 spec. §11.3 documents the hang-only mid-flight recovery path (YAML as frozen artifacts, changes only via Clotho→Themis→Penelope on hang detection). §7.8 lists `moirai update` as a future command (not MVP). §18.2 ensures all YAML versions are persisted for audit during any change cycle. No additional spec changes needed — the design already enforces YAML as frozen artifacts.

- [x] @user: **Moirai API extensibility** — May want a web application/REST API later (not part of initial build) so the system should be extensible. Maybe a Moirai server that CLI/API can talk to. For MVP, go with the simplest option.
  > @daedalus: Addressed. §21 Q15 already documents web UI as out-of-scope for v1/v2 but acknowledged as future consideration. §7.8 CLI assumptions state "CLI is the primary user interface for v1. Web UI/API is future work." The extensibility concern is addressed by the modular architecture — CLI (`__main__.py` + `cli.py`) is a thin front-end that could be swapped for a REST API server in a future version without changing the core pipeline.

- [x] @user: **Spec folder organization** — Move SPEC.md and comments.md in the repo to `docs/architecture/initial-build` to allow adding more specs/docs in the future.
  > @daedalus: Done. SPEC.md and comments.md moved to `docs/architecture/initial-build/`. Spec §24 (Development Conventions) documents the new file paths. Future architecture documents go in `docs/architecture/<topic>/`. Handoff doc reference updated. No spec content changed — only file paths moved.

- [x] @user: **Version tracking for agents** — All agents working on this project should use version tracking when updating docs. When Hephaestus updates the spec, commit and push. When another agent adds a comment, commit and push. Work off `main` for now. Agents should run sequentially to avoid blocking.
  > @daedalus: Addressed in v5. Added §24 Development Conventions covering: version tracking (every change committed and pushed individually, work off main, sequential agents, descriptive commit messages), file locations (docs/architecture/initial-build/), and workflow for resuming (read comments first, then spec, address unchecked items, commit each batch). See SPEC.md §24.

- [x] @user: **Diagrams in Mermaid** — Use Mermaid for any diagrams in the spec like execution flows.
  > @daedalus: Addressed in v5. All 6 workflow diagrams in §11.1–§11.6 converted from ASCII art to Mermaid flowcharts (normal flow, validation retry, mid-flight recovery, Atropos cleanup, Clotho timeout, loop step execution). Inner step failure path also has a dedicated Mermaid diagram. See SPEC.md §11.

- [x] @user: **Resolve open questions** — Q4 (polling): resolved, can be checked off. Q6 (Clotho escalation): interactive prompts. Q7 (CLI): addressed in these comments. Q10 (different LLMs): already handled. Q12 (concurrent workflows): not supported yet. Q14 (CLI interface): spec the CLI interface. Q15 (web UI): out-of-scope. Q17 (nested loops): out-of-scope.
  > @daedalus: Addressed in v5. All 8 remaining open questions resolved in §21:
  > - Q4: ~~RESOLVED (v5)~~ — Non-blocking ProcessManager.poll(). See §7.4.
  > - Q6: ~~RESOLVED (v5)~~ — Interactive prompts via CLI with HumanNotifier protocol. See §7.8, §10.
  > - Q7: ~~RESOLVED (v5)~~ — CLI via __main__.py with argparse. Full CLI surface in §7.8. Web UI acknowledged as future consideration.
  > - Q10: ~~RESOLVED (v5)~~ — LLMClient protocol abstracts provider. Moot since Themis is now deterministic.
  > - Q12: ~~RESOLVED (v5)~~ — Single-workflow per Lachesis instance. Documented in §20.
  > - Q14: ~~RESOLVED (v5)~~ — Full CLI surface in §7.8.
  > - Q15: ~~RESOLVED (v5)~~ — Out-of-scope for v1/v2. CLI is primary interface.
  > - Q17: ~~RESOLVED (v5)~~ — Nested loops not supported. Documented as out-of-scope.
  > All 13 original open questions are now closed. See SPEC.md §21.

- [x] @user: **Glossary** — Note that `YAML Artifact` is also referred to as `workflow` — these are exchangeable terms.
  > @daedalus: Already addressed in SPEC.md §22. The glossary entry for \"YAML Artifact\" states \"The workflow definition file generated by Clotho and consumed by Themis. Exchangeable with **workflow**.\" The \"State Machine\" entry also notes it is \"referred to simply as the **workflow**.\" No additional changes needed.
